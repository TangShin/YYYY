...
- (void)viewDidLoad
{
    ...
    [YYYHttpTool post:YYYMVhomeURL params:nil success:^(id json) {
        
        _next = [json[@"mv"][@"next"] intValue];
        _page = json[@"mv"][@"page"];
        _sortDiv = json[@"mv"][@"sortDiv"];
        _mvListData = json[@"mv"][@"mvList"];
...
- (void)buildUI
{
    ...
    //显示歌单的collectionview
    _mvCollectionView = [[MVCollectionViewController alloc] init];
    [_mvCollectionView.collectionView setBackgroundColor:[UIColor whiteColor]];
    [_mvCollectionView.collectionView setFrame:CGRectMake(0, Height,kScreenWidth,kScreenHeight - Height)];
    
    //给collection传值
    _mvCollectionView.sortDiv = _sortDiv;
    [_mvCollectionView.mvListData addObjectsFromArray:_mvListData];
    
    //集成刷新控件
    [self addHeader:_currentButtonTag];
    [self addFooter];
    
    [self addChildViewController:_mvCollectionView];
    [self.view addSubview:_mvCollectionView.collectionView];
...
{
    _mvCollectionView.collectionView.header = [MJRefreshNormalHeader headerWithRefreshingBlock:^{
        
        [self loadPlaylistData:currentButtonTag];
        [_mvCollectionView.collectionView.header endRefreshing];
        
    }];
    
    [_mvCollectionView.collectionView.header beginRefreshing];
}
...
{
    if (_next) {
        _mvCollectionView.collectionView.footer = [MJRefreshAutoNormalFooter footerWithRefreshingBlock:^{
            ...
            if (_next) {
                [YYYHttpTool post:YYYMVhomeURL params:params success:^(id json) {
                    
                    _next = [json[@"mv"][@"next"] intValue];
                    _page = json[@"mv"][@"page"];
                    _sortDiv = json[@"mv"][@"sortDiv"];
                    
                    for (NSDictionary *dic in json[@"mv"][@"mvList"]) {
                        [_mvCollectionView.mvListData addObject:dic];
                    }
                    
                    [_mvCollectionView.collectionView reloadData];
                    [_mvCollectionView.collectionView.footer endRefreshing];
                    
                } failure:^(NSError *error) {
                    TSLog(@"FooterRefresh %@",error);
                }];
            } else {
                [_mvCollectionView.collectionView.footer endRefreshingWithNoMoreData];
            }
...
- (void)loadPlaylistData:(NSInteger)tag
{
    ...
    [YYYHttpTool post:YYYMVhomeURL params:params success:^(id json) {
        ...
        [_mvCollectionView.mvListData removeAllObjects];
        for (NSDictionary *dict in json[@"mv"][@"mvList"]) {
            [_mvCollectionView.mvListData addObject:dict];
        }
        
        [_mvCollectionView.collectionView.footer removeFromSuperview];
        [self addFooter];
        [_mvCollectionView.collectionView reloadData];
...
- (void)sortPlaylist
{
    ...
    UIAlertAction *normalSortAction = [UIAlertAction actionWithTitle:@"按默认排序" style:(UIAlertActionStyleDefault) handler:^(UIAlertAction *action) {
        // 给定义的全局变量赋值
        if (![self.sortDiv isEqualToString:VIDEO_SORT_TIME]) {
            self.sortDiv = VIDEO_SORT_TIME;
            [self addHeader:_currentButtonTag];
        }
    }];
    // 添加收藏排序按钮
    UIAlertAction *favoriteSortAction = [UIAlertAction actionWithTitle:@"按收藏排序" style:(UIAlertActionStyleDefault) handler:^(UIAlertAction *action) {
        if (![self.sortDiv isEqualToString:VIDEO_SORT_FAVORITE_COUNT]) {
            self.sortDiv = VIDEO_SORT_FAVORITE_COUNT;
            [self addHeader:_currentButtonTag];
        }
    }];
    // 添加点赞排序按钮
    UIAlertAction *likeSortAction = [UIAlertAction actionWithTitle:@"按点赞排序" style:(UIAlertActionStyleDefault) handler:^(UIAlertAction *action) {
        if (![self.sortDiv isEqualToString:VIDEO_SORT_LIKE_COUNT]) {
            self.sortDiv = VIDEO_SORT_LIKE_COUNT;
            [self addHeader:_currentButtonTag];
        }
    }];
    // 添加播放排序按钮
    UIAlertAction *playSortAction = [UIAlertAction actionWithTitle:@"按播放排序" style:(UIAlertActionStyleDefault) handler:^(UIAlertAction *action) {
        if (![self.sortDiv isEqualToString:VIDEO_SORT_PLAY_COUNT]) {
            self.sortDiv = VIDEO_SORT_PLAY_COUNT;
            [self addHeader:_currentButtonTag];
        }
    }];
    ...
    [alertController addAction:playSortAction];
