...
@property (nonatomic,strong) YYYPlayCustomNavigationView *navigationView;//顶部工具条
@property (nonatomic,strong) YYYPlayCustomControlView *controlview; //底部工具条
...
//视频播放数组下标
@property (assign,nonatomic) long videoTag;
//区分上下view的手势事件
...
- (void)viewDidLoad {
    ...
    for (NSDictionary *dict in [ProfileViewTool videoHistory].videoHistory) {
        [_videoHistoryArray addObject:dict];
    }
    
    [self loadPlaylistData:self.playlistId];
...
#pragma mark 加载歌单数据
- (void)loadPlaylistData:(NSString *)playlist
{
    NSMutableDictionary *prametersPlaylist = [NSMutableDictionary dictionaryWithCapacity:1];
    [prametersPlaylist setObject:playlist forKey:@"playlistId"];
    
    [YYYHttpTool post:YYYLoadPlaylistURL params:prametersPlaylist success:^(id json) {
        
        _commentDic = [json valueForKey:@"media"];
        _playlistDic = [json valueForKey:@"playlist"];
        _videoArray = _playlistDic[@"playlistVideo"];
        ...
        TSLog(@"loadPlaylistERROR__%@",error);
...
- (void)buildPlayVideoUI
{
    ...
    NSDictionary *videoInfoDic = self.videoArray[_videoTag];
    ...
    if (self.videoVC) {
        ...
        self.videoVC.videoArray = self.videoArray;
    ...
    self.videoVC.videoUrl = [NSString stringWithFormat:@"%@",videoInfoDic[@"videoAddr"]];
    self.videoVC.videoName = videoInfoDic[@"videoName"];
    self.videoVC.videoId = videoInfoDic[@"videoId"];
    
    //创建起始视频占为图
    if (self.videoPoster) {
        self.videoPoster.hidden = NO;
    }else {
        self.videoPoster = [[UIImageView alloc] init];
        self.videoPoster.frame = self.videoVC.view.frame;
        [self.view addSubview:self.videoPoster];
    }
    [self.videoPoster sd_setImageWithURL:videoInfoDic[@"videoPreviewAddr"]];
    
    //创建未全屏时视频顶部工具条
    [self setupTopView:videoInfoDic];
    ...
    if (self.playButton) {
        ...
        CGFloat playButtonX = CGRectGetMaxX(self.videoPoster.frame) - 60;
        CGFloat playButtonY = CGRectGetMaxY(self.videoPoster.frame) - 60;
...
#pragma mark 创建未全屏时视频顶部工具条
- (void)setupTopView:(NSDictionary *)videoInfoDic
{
    if (_navigationView) {
        _navigationView.title.text = videoInfoDic[@"videoName"];
    } else {
        ...
        if (nibArray.count > 0) {
            ...
            view.title.text = videoInfoDic[@"videoName"];
...
- (void)initTopTabs
{
    ...
    _currentTag = 0;
    [self TopTabsButtonWithButton:_playlistsBtn Name:@"歌单" tag:1];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView.tag == 0 && _playlistDic != nil) {//歌单简介的tableview
        if (indexPath.row == 0) {
            VideoShareCell *cell = [tableView dequeueReusableCellWithIdentifier:@"ShareCell"];
            cell.shareCountLabel.text = [NSString stringWithFormat:@"%@",_playlistDic[@"shareCount"]];
            cell.favoriteCountLabel.text = [NSString stringWithFormat:@"%@",_playlistDic[@"favoriteCount"]];
            cell.likeCountLabel.text = [NSString stringWithFormat:@"%@",_playlistDic[@"likeCount"]];
            cell.downloadCountLabel.text = [NSString stringWithFormat:@"%@",_playlistDic[@"downloadCount"]];
...
//            [cell.downloadBtn addTarget:self action:@selector(downloadClick:) forControlEvents:UIControlEventTouchUpInside];
            ...
            [iconImage sd_setImageWithURL:_playlistDic[@"uploadUserPhoto"]];
            cell.upUserPhoto.image = [self scaleImageSize:CGSizeMake(50, 50) image:iconImage];
            cell.upUserName.text = _playlistDic[@"uploadUserName"];
            cell.upLoadTime.text = _playlistDic[@"uploadTimeView"];
            cell.upUserPhoto.layer.masksToBounds = YES;
            cell.upUserPhoto.layer.cornerRadius = cell.upUserPhoto.bounds.size.width/2;
            bool followBool = [_playlistDic[@"uploadUserFollowed"] integerValue];
            if (followBool) {
...
//            [cell.followed addTarget:self action:@selector(followCancelCilick:) forControlEvents:UIControlEventTouchUpInside];
            ...
            cell.videoName.text = _playlistDic[@"videoName"];
            cell.playCount.text = _playlistDic[@"playCount"];
            cell.comentCount.text = _playlistDic[@"commentCount"];
//            if (_showTextState == YES) {
//                cell.videoInfo.numberOfLines = 0;
//                cell.videoInfo.lineBreakMode = NSLineBreakByClipping;
//            } else {
                cell.videoInfo.text = [self decodeFromHTML:_playlistDic[@"videoIntroduction"]];
//            }
//            [cell.showMore addTarget:self action:@selector(showMoreText:) forControlEvents:UIControlEventTouchUpInside];
    ...
    else if (tableView.tag == 1 && _videoArray != nil) {//推荐视频的tableview
        RcomdViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"RcomdCell"];
        [cell.videoImg sd_setImageWithURL:_videoArray[indexPath.row][@"videoPreviewAddr"]];
        cell.videoImg.layer.masksToBounds = YES;
        cell.videoImg.layer.cornerRadius = 5;
        cell.title.text = _videoArray[indexPath.row][@"videoName"];
        cell.userName.text = _videoArray[indexPath.row][@"userName"];
        cell.playCount.text = _videoArray[indexPath.row][@"playCount"];
        cell.comentCount.text = _videoArray[indexPath.row][@"commentCount"];
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        return cell;
...
{
    if (tableView.tag == 1) {
        [self.videoVC switchVideoUrl:_videoArray[indexPath.row] indexPath:indexPath];
        self.videoPoster.hidden = YES;
        self.playButton.hidden = YES;
        _videoTag = indexPath.row;
    } else if (tableView.tag == 2) {
        self.splitTap = NO;
        ...
        subCommentView.videoId = _playlistId;
...
-(void)loadMoreData
{
    ...
    commentTableview.header=[MJRefreshNormalHeader headerWithRefreshingBlock:^{
        ...
        prametersRefish = [NSDictionary dictionaryWithObjectsAndKeys:self.playlistId,@"mediaId",@"1",@"pageButton",nil];
    ...
    if (self.next) {
        commentTableview.footer=[MJRefreshAutoNormalFooter footerWithRefreshingBlock:^{
            if (self.next) {
                ...
                prametersMediapaging = [NSDictionary dictionaryWithObjectsAndKeys:self.playlistId,@"mediaId",@"下页",@"pageButton",self.commentPage,@"page",nil];
...
#pragma mark - 所有的点击事件在这
#pragma mark 播放视频按钮点击事件
- (void)playVideo
{
    [self videoHistorySave];
    TSLog(@"%@",[ProfileViewTool videoHistory].videoHistory);
...
- (void)videoHistorySave
{
    for (int i = 0; i < _videoHistoryArray.count; i++) {
        ...
        if ([_playlistId isEqualToString:videoIdHistory]) {
            [_videoHistoryArray removeObjectAtIndex:i];
            break;
        }
    }
    [_videoHistoryArray addObject:[self videoHistoryDicDIY:_playlistDic isPlaylist:YES]];
...
- (void)sendComment:(UIButton *)sender
{
    ...
    if (userInfo.userId == nil) {
        ...
        prameters = [NSDictionary dictionaryWithObjectsAndKeys:self.playlistId,@"mediaId",self.TextViewInput.text,@"content",userInfo.userId,@"userId",nil];
