...
#error YYYWeakTimer is ARC only. Either turn on ARC for the project or use -fobjc-arc flag
#endif

#if OS_OBJECT_USE_OBJC
#define yyy_gcd_property_qualifier strong
#define yyy_release_gcd_object(object)
#else
#define yyy_gcd_property_qualifier assign
#define yyy_release_gcd_object(object) dispatch_release(object)
#endif

@interface YYYWeakTimer ()
{
    struct
    {
...
@property (nonatomic, yyy_gcd_property_qualifier) dispatch_queue_t privateSerialQueue;

@property (nonatomic, yyy_gcd_property_qualifier) dispatch_source_t timer;

@end

@implementation YYYWeakTimer
...
- (id)init
{
    return [self initWithTimeInterval:0 target:nil selector:NULL userInfo:nil repeats:NO dispatchQueue:nil];
}
...
{
    YYYWeakTimer *timer = [[self alloc] initWithTimeInterval:timeInterval target:target selector:selector userInfo:userInfo repeats:repeats dispatchQueue:dispatchQueue];
...
- (void)dealloc
{
    ...
    yyy_release_gcd_object(_privateSerialQueue);
}
...
- (void)resetTimerProperties
{
    ...
    dispatch_source_set_timer(self.timer, dispatch_time(DISPATCH_TIME_NOW, intervalInNanoseconds), (uint64_t)intervalInNanoseconds, toleranceInNanoseconds);
}
...
- (void)schedule
{
    ...
    __weak YYYWeakTimer *weakSelf = self;
